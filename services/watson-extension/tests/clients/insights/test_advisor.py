...
async def test_find_rules(client, aiohttp_mock) -> None:
    ...
    assert (
        response.link
        == "/insights/advisor/recommendations?impacting=true&rule_status=enabled"
    )
    assert response.rules[0].id == "ae_eol|AE_EOL_ERROR"
    assert (
        response.rules[0].description
    ...
    assert (
        response.rules[0].link
        == "/insights/advisor/recommendations/ae_eol|AE_EOL_ERROR"
    )
...
async def test_find_rules_with_category_id(client, aiohttp_mock) -> None:
    ...
    assert (
        response.link
        == "/insights/advisor/recommendations?impacting=true&rule_status=enabled&category=4"
    )
    assert response.rules[0].id == "aws_nvme_io_timeout|AWS_NVME_IO_TIMEOUT_V2"
    assert (
        response.rules[0].description
        == "The NVMe driver fails I/O when I/O latency exceeds operation timeout on Amazon EC2 instances"
    )
    assert (
        response.rules[0].link
        == "/insights/advisor/recommendations/aws_nvme_io_timeout|AWS_NVME_IO_TIMEOUT_V2"
    )
...
async def test_find_rules_with_total_risk(client, aiohttp_mock) -> None:
    ...
    assert (
        response.link
        == "/insights/advisor/recommendations?impacting=true&rule_status=enabled&total_risk=4"
    )
    assert (
        response.rules[0].id
        == "kernel_panic_missing_glibc|KERNEL_PANIC_MISSING_GLIBC_WARN"
    )
    assert (
        response.rules[0].description
    ...
    assert (
        response.rules[0].link
        == "/insights/advisor/recommendations/kernel_panic_missing_glibc|KERNEL_PANIC_MISSING_GLIBC_WARN"
    )
...
async def test_find_rules_with_sort_and_only_workloads(client, aiohttp_mock) -> None:
    ...
    response = await client.find_rules(
        sort=FindRuleSort.PublishDate, only_workloads=True
    )
    assert len(response.rules) == 3
    assert (
        response.link
        == "/insights/advisor/recommendations?impacting=true&rule_status=enabled&sort=-publish_date&filter[system_profile][sap_system]=true"
    )
    assert (
        response.rules[0].id
        == "hardening_ssh_hmac_cipher_rhel8_later|SSH_INSECURE_HMAC_CIPHER_RHEL8_LATER_CRYPTO"
    )
    assert (
        response.rules[0].description
    ...
    assert (
        response.rules[0].link
        == "/insights/advisor/recommendations/hardening_ssh_hmac_cipher_rhel8_later|SSH_INSECURE_HMAC_CIPHER_RHEL8_LATER_CRYPTO"
    )
