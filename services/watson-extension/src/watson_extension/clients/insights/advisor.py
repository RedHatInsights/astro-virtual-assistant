...
class AdvisorClientHttp(AdvisorClient):
    def __init__(self, advisor_url: injector.Inject[AdvisorURL], user_identity_provider: injector.Inject[AbstractUserIdentityProvider], platform_request: injector.Inject[AbstractPlatformRequest]):
        super().__init__()
    ...
    async def find_rule_category_by_name(self, category_name: str) -> RuleCategory:
        response = await self.platform_request.get(self.advisor_url, "/api/insights/v1/rulecategory/", user_identity=await self.user_identity_provider.get_user_identity())
    ...
    async def find_rules(self, category_id: Optional[str] = None, total_risk: Optional[int] = None, sort: Optional[FindRuleSort] = None,
        ...
        response = await self.platform_request.get(self.advisor_url, request, user_identity=await self.user_identity_provider.get_user_identity())
        response.raise_for_status()
