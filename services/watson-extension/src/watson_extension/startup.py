...
from watson_extension.clients.identity import QuartUserIdentityProvider, AbstractUserIdentityProvider, FixedUserIdentityProvider

@injector.provider
def dev_platform_request(session: injector.Inject[aiohttp.ClientSession]) -> DevPlatformRequest:
...
def injector_from_config(binder: injector.Binder) -> None:
    ...
    if config.is_running_locally:
        binder.bind(AbstractUserIdentityProvider, FixedUserIdentityProvider, scope=injector.singleton)
    else:
        # This injector is per request - as we should extract the data for each request.
        binder.bind(AbstractUserIdentityProvider, QuartUserIdentityProvider, scope=quart_injector.RequestScope)

    if config.platform_request == "dev":
        binder.bind(AbstractPlatformRequest, to=dev_platform_request, scope=injector.singleton)
    else:
        binder.bind(AbstractPlatformRequest, to=PlatformRequest, scope=injector.singleton)
